...
export function MechanicDetailChart({ mechanic, data = [], loading = false }) {
  ...
  const chartOption = useMemo(() => {
    ...
    return {
      ...
      grid: { left: 90, right: 40, bottom: 32, top: 108, containLabel: true },
      xAxis: {
        type: 'category',
        boundaryGap: false,
        data: categories,
      ...
      yAxis: {
        type: 'value',
        axisLabel: { color: COLORS.text, margin: 14 },
    ...
    <Card className="border-border shadow-sm bg-card/80">
      <CardHeader className="space-y-1.5 pb-0">
        <CardTitle className="text-lg">
          {hasMechanic ? `Evolução – ${mechanic.nome}` : 'Evolução por Mecânico'}
        </CardTitle>
        <p className="text-xs text-muted-foreground">Desempenho mensal de budgets e serviços.</p>
      </CardHeader>
